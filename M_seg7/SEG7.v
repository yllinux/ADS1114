...
*****************************************************************************/
module SEG7 (clk, rst_n, key2, data, seg, sel);
    ...
    //wire    [3:0]    baif;
    //wire    [3:0]    qianf;
    wire    [9:0]   result_100; //结果的100倍
    wire    [15:0]  result;     //结果100倍的BCD码
    assign result_100   =  (data[15]) ? ((26869350 - 410*data) / 32767) :  ((410 * data) / 32767);
    //assign ge    =  result_1000 / 1000;
    //assign shif  =  (result_1000 % 1000) / 100;
    //assign baif  =  ((result_1000 % 1000) % 100) / 10;
    //assign qianf =  ((result_1000 % 1000) % 100) % 10;
                ...
                if (cnt >= 200)      //输入的数据每计数到300 ，显示一次。
                    begin 
                        if (key2 == 4'd0)
                            ...
                            data_r  <=  result;   //{ge, shif, baif, qianf};
                        cnt     <=  12'd0;
                    end 
                else 
                    cnt   <=  cnt + 12'd1;
    ...
    always @(posedge clk or negedge rst_n)
        begin 
            if(!rst_n)
                ...
                begin 
                    case (st)    //选择数码管
                        3'd2 :                                 //将第 [3:0]; 位数显示在从左至右第 4 个数码管上
                            begin 
                                sel        <=  6'b11_1011;
                        ...
                        3'd0 :                                 //将第 [11:8] 位数显示在从左至右第 2 个数码管上
                            begin 
                                sel        <=  6'b11_1110;
